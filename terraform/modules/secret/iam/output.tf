...
output "eks_oidc_role_arn" {
  ...
  value       = (var.is_eks_cluster_enabled && var.eks_oidc_url != "") ? aws_iam_role.eks_oidc[0].arn : null
}

output "eks_oidc_role_name" {
  description = "Name of the EKS OIDC IAM role"
  value       = (var.is_eks_cluster_enabled && var.eks_oidc_url != "") ? aws_iam_role.eks_oidc[0].name : null
}

output "eks_oidc_policy_arn" {
  description = "ARN of the EKS OIDC IAM policy"
  value       = (var.is_eks_cluster_enabled && var.eks_oidc_url != "") ? aws_iam_policy.eks-oidc-policy[0].arn : null
}

output "eks_oidc_provider_arn" {
  description = "ARN of the EKS OIDC identity provider"
  value       = (var.is_eks_cluster_enabled && var.eks_oidc_url != "") ? aws_iam_openid_connect_provider.eks-oidc-provider[0].arn : null
}

output "eks_oidc_provider_url" {
  description = "URL of the EKS OIDC identity provider"
  value       = (var.is_eks_cluster_enabled && var.eks_oidc_url != "") ? aws_iam_openid_connect_provider.eks-oidc-provider[0].url : null
}
